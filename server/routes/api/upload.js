    ...
    .post(async (req, res) => {
        try {
            ...
            csv({
                ...
                .then(function(stockUpload) {
                    stockUpload
                    .save()
                    .then(() => {
                        if(file.name === undefined) {
                            res.json({ msg: 'File upload errors', errors: 'upload file error' });
                        }
                        res.json({ fileName: file.name });
                    })
                    .catch(err => {
                        console.log('csv file upload problem', err);
                        res.json({ msg: 'File upload errors', errors: err });
                    })
                })
                .catch(function(err) {
